...
while [[ $DBSTATUS -ne 0 ]] && [[ $i -lt 60 ]]; do #&& [[ $ERRCODE -ne 0 ]]; do # Removecheck to ERRCODE since it always returns 0 (and this is a proof of concept)
	i=$((i+1))
	echo ".:: Waiting for SQL Server to start (Try: $i)... ::."
	DBSTATUS=$(/opt/mssql-tools/bin/sqlcmd -h -1 -t 1 -U sa -P $SA_PASSWORD -Q "SET NOCOUNT ON; Select SUM(state) from sys.databases")
	ERRCODE=$?
	echo ".:: TEST: DBSTATUS: $DBSTATUS ERRCODE: $ERRCODE ::."
	sleep 1
done
